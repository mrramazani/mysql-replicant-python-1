...
class TestConfigFile(unittest.TestCase):
    ...
    def __init__(self, methodName, options):
        super(TestConfigFile, self).__init__(methodName)
...
def suite(options={}):
    ...
    for test in unittest.defaultTestLoader.getTestCaseNames(TestConfigFile):
        suite.addTest(TestConfigFile(test, options=options))
    return suite
